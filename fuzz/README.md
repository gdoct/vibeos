...
functionlinebranchtcp_input78%69%tcp_recv_data100%90%tcp_recv_ack100%88%tcp_reassemble100%100%tcp_ooo_store100%93%tcp_parse_mss100%100%tcp_rx_append100%75%snd_copyout100%100%tcp_output80%59%tcp_timer80%64%tcp_enter_timewait0%0%
...
Debunked (was my error, not yours): the "infinite loop" I first hit was a harness artifact. A RST to an un-cached host spun in arp_resolve — but only because my stub made wait_queue_sleep_locked a no-op and froze the clock. The real kernel sleeps while the timer ISR advances time, so that while (… && timer_ticks() < deadline) bounds out after ~0.5s×5 and ARP fails cleanly. Fixed the stub to advance the clock during sleep; the input then returns normally. Worth flagging as a reminder that a fuzzer finding is only as trustworthy as the harness.
